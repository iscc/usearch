...
template <typename native_at> struct dense_index_py_gt : public native_at {
    using native_t = native_at;
    using native_t::add;
    ...
    dense_index_py_gt(native_t&& base) : native_t(std::move(base)) {}
};

using dense_index_py_t = dense_index_py_gt<index_dense_t>;
using dense_index_big_py_t = dense_index_py_gt<index_dense_big_t>;

template <typename index_py_t>
struct dense_indexes_py_gt {
    using native_t = typename index_py_t::native_t;
    using index_t = index_py_t;
    std::vector<std::shared_ptr<index_py_t>> shards_;

    void merge(std::shared_ptr<index_py_t> shard) { shards_.push_back(shard); }
    ...
    void merge_paths(std::vector<std::string> const& paths, bool view = true, std::size_t threads = 0) {
        ...
        executor_default_t{threads}.dynamic(paths.size(), [&](std::size_t thread_idx, std::size_t task_idx) {
            native_t index = native_t::make(paths[task_idx].c_str(), view);
            if (!index)
                return false;
            auto shared_index = std::make_shared<index_py_t>(std::move(index));
            std::unique_lock<std::mutex> lock(shards_mutex);
            shards_.push_back(shared_index);
            // Only check signals from the main thread (thread 0) to avoid
            // crashes on Windows where PyErr_CheckSignals requires the GIL.
            if (thread_idx == 0)
...
using dense_indexes_py_t = dense_indexes_py_gt<dense_index_py_t>;
using dense_indexes_big_py_t = dense_indexes_py_gt<dense_index_big_py_t>;

template <typename index_at> struct index_key_gt {
    using key_t = typename index_at::vector_key_t;
};

template <typename index_py_t>
struct index_key_gt<dense_indexes_py_gt<index_py_t>> {
    using key_t = typename index_key_gt<index_py_t>::key_t;
};

template <typename key_at> struct key_codec_gt;

template <> struct key_codec_gt<index_dense_t::vector_key_t> {
    using key_t = index_dense_t::vector_key_t;

    static key_t from_object(py::handle object) { return object.cast<key_t>(); }

    static py::object to_object(key_t key) { return py::int_(key); }

    static py::dtype dtype() { return py::dtype::of<key_t>(); }

    static key_t free_key() { return default_free_value<key_t>(); }

    static void validate_keys_buffer(py::buffer_info const& keys_info) {
...
template <> struct key_codec_gt<unum::usearch::uuid_t> {
    using key_t = unum::usearch::uuid_t;

    static key_t from_object(py::handle object) {
        if (!PyObject_CheckBuffer(object.ptr()))
            throw std::invalid_argument("128-bit keys must be bytes-like objects of length 16.");

        py::buffer_info info = py::reinterpret_borrow<py::buffer>(object).request();
        if (info.itemsize != 1 || info.ndim != 1 || info.shape[0] != 16 || info.strides[0] != 1)
            throw std::invalid_argument("128-bit keys must be bytes-like objects of length 16.");

        key_t key {};
        std::memcpy(key.octets, info.ptr, 16);
        return key;
    }

    static py::object to_object(key_t const& key) { return py::bytes(reinterpret_cast<char const*>(key.octets), 16); }

    static py::dtype dtype() { return py::dtype("V16"); }

    static key_t free_key() {
        key_t key {};
        std::memset(key.octets, 0xFF, 16);
        return key;
    }

    static void validate_keys_buffer(py::buffer_info const& keys_info) {
        if (keys_info.itemsize != 16)
            throw std::invalid_argument("128-bit keys must use 16-byte items (e.g. NumPy '|V16').");
        if (keys_info.ndim != 1)
            throw std::invalid_argument("Keys must be placed in a single-dimensional array!");
        if (keys_info.strides[0] != 16)
            throw std::invalid_argument("128-bit keys array must be C-contiguous with stride 16.");
    }
};

template <typename key_at> static py::array make_keys_array(py_shape_t const& shape) {
    return py::array(key_codec_gt<key_at>::dtype(), shape);
}

template <typename key_at>
static key_at* key_data(py::buffer_info const& keys_info) {
    return reinterpret_cast<key_at*>(keys_info.ptr);
}

template <typename index_py_at>
static index_py_at make_index(               //
    ...
    using native_t = typename index_py_at::native_t;
    using key_t = typename native_t::vector_key_t;
    ...
    using index_state_t = typename native_t::state_result_t;
    index_state_t state = native_t::make(metric, config, key_codec_gt<key_t>::free_key());
    if (!state)
...
static void add_typed_to_index(                                            //
    ...
    using key_t = typename index_at::vector_key_t;
    using add_result_t = typename index_at::add_result_t;
    ...
    executor_default_t{threads}.dynamic(vectors_count, [&](std::size_t thread_idx, std::size_t task_idx) {
        ...
        add_result_t result = index.add(key, vector, thread_idx, force_copy);
        if (!result) {
...
static void add_many_to_index(                            //
    ...
    key_codec_gt<key_t>::validate_keys_buffer(keys_info);
...
template <typename scalar_at, typename index_at, typename key_at = typename index_at::vector_key_t>
static void search_typed(                                   //
    index_at& index, py::buffer_info& vectors_info,         //
    std::size_t wanted, bool exact, std::size_t threads,    //
    py::array& keys_py, py::array_t<distance_t>& distances_py, py::array_t<Py_ssize_t>& counts_py,
    std::atomic<std::size_t>& stats_visited_members, std::atomic<std::size_t>& stats_computed_distances,
    progress_func_t const& progress) {

    using key_t = key_at;
    using search_result_t = typename index_at::search_result_t;

    py::buffer_info keys_info = keys_py.request();
    key_t* keys_data = key_data<key_t>(keys_info);
    ...
    executor_default_t{threads}.dynamic(vectors_count, [&](std::size_t thread_idx, std::size_t task_idx) {
        ...
        search_result_t result = index.search(vector, wanted, thread_idx, exact);
        if (!result) {
            atomic_error = result.error.release();
            return false;
        }

        key_t* query_keys = reinterpret_cast<key_t*>(reinterpret_cast<byte_t*>(keys_data) + task_idx * keys_info.strides[0]);
        counts_py1d(task_idx) = static_cast<Py_ssize_t>(result.dump_to(query_keys, &distances_py2d(task_idx, 0), wanted));
...
template <typename scalar_at, typename indexes_at>
static void search_typed_multi(                              //
    indexes_at& indexes, py::buffer_info& vectors_info,      //
    std::size_t wanted, bool exact, std::size_t threads,     //
    py::array& keys_py, py::array_t<distance_t>& distances_py, py::array_t<Py_ssize_t>& counts_py,
    std::atomic<std::size_t>& stats_visited_members, std::atomic<std::size_t>& stats_computed_distances,
    progress_func_t const& progress) {

    using key_t = typename index_key_gt<indexes_at>::key_t;
    using index_py_t = typename indexes_at::index_t;
    using search_result_t = typename index_py_t::search_result_t;
    ...
    executor_default_t{threads}.dynamic(indexes.shards_.size(), [&](std::size_t thread_idx, std::size_t task_idx) {
        index_py_t& index = *indexes.shards_[task_idx].get();
        ...
        for (std::size_t vector_idx = 0; vector_idx != static_cast<std::size_t>(vectors_count); ++vector_idx) {
            ...
            search_result_t result = index.search(vector, wanted, 0, exact);
            if (!result) {
                atomic_error = result.error.release();
                return false;
            }

            {
                ...
                key_t* query_keys =
                    reinterpret_cast<key_t*>(reinterpret_cast<byte_t*>(keys_data) + vector_idx * keys_info.strides[0]);
            ...
            ++processed;
            if (thread_idx == 0) {
                std::size_t total = static_cast<std::size_t>(vectors_count) * indexes.shards_.size();
                if (PyErr_CheckSignals() != 0 || !progress_(processed.load(), total)) {
                    atomic_error.store("Operation has been terminated");
                    return false;
                }
            }
    ...
    // At the end report the latest numbers, because the reporter thread may be finished earlier
    std::size_t total = static_cast<std::size_t>(vectors_count) * indexes.shards_.size();
    progress_(processed.load(), total);
...
static py::tuple search_many_in_index( //
    ...
    using key_t = typename index_key_gt<index_at>::key_t;
    ...
    py::array keys_py = make_keys_array<key_t>(py_shape_t {vectors_count, static_cast<Py_ssize_t>(wanted)});
 ...
 *      4. number of computed pairwise distances.
 */
template <typename indexes_at>
static py::tuple search_many_in_indexes( //
    indexes_at& indexes, py::buffer vectors, std::size_t wanted, bool exact, std::size_t threads,
    progress_func_t const& progress) {
    ...
    Py_ssize_t vectors_count = vectors_info.shape[0];
    Py_ssize_t vectors_dimensions = vectors_info.shape[1];
    if (vectors_dimensions != static_cast<Py_ssize_t>(indexes.scalar_words()))
        throw std::invalid_argument("The number of vector dimensions doesn't match!");
    if (vectors_info.strides[1] != static_cast<Py_ssize_t>(vectors_info.itemsize))
        throw std::invalid_argument("Matrix rows must be contiguous, try `ascontiguousarray`.");
    ...
    // clang-format off
    switch (numpy_string_to_kind(vectors_info.format)) {
    case scalar_kind_t::b1x8_k: search_typed_multi<b1x8_t>(indexes, vectors_info, wanted, exact, threads, keys_py, distances_py, counts_py, stats_visited_members, stats_computed_distances, progress); break;
    case scalar_kind_t::i8_k: search_typed_multi<i8_t>(indexes, vectors_info, wanted, exact, threads, keys_py, distances_py, counts_py, stats_visited_members, stats_computed_distances, progress); break;
    case scalar_kind_t::f16_k: search_typed_multi<f16_t>(indexes, vectors_info, wanted, exact, threads, keys_py, distances_py, counts_py, stats_visited_members, stats_computed_distances, progress); break;
    case scalar_kind_t::f32_k: search_typed_multi<f32_t>(indexes, vectors_info, wanted, exact, threads, keys_py, distances_py, counts_py, stats_visited_members, stats_computed_distances, progress); break;
    case scalar_kind_t::f64_k: search_typed_multi<f64_t>(indexes, vectors_info, wanted, exact, threads, keys_py, distances_py, counts_py, stats_visited_members, stats_computed_distances, progress); break;
    default: throw std::invalid_argument("Incompatible scalars in the query matrix: " + vectors_info.format);
    }
    // clang-format on
...
static py::tuple cluster_vectors(        //
    ...
    using key_t = typename index_at::vector_key_t;
    using clustering_result_t = typename index_at::clustering_result_t;
    ...
    py::array keys_py = make_keys_array<key_t>(py_shape_t {Py_ssize_t(queries_count), Py_ssize_t(1)});
    ...
    py::buffer_info keys_info = keys_py.request();
    auto distances_py2d = distances_py.template mutable_unchecked<2>();
    key_t* keys_ptr = key_data<key_t>(keys_info);
...
static py::tuple cluster_keys(                            //
    index_at& index, py::buffer queries_py,               //
    std::size_t min_count, std::size_t max_count, std::size_t threads, progress_func_t const& progress) {
    ...
    py::buffer_info queries_info = queries_py.request();
    key_codec_gt<key_t>::validate_keys_buffer(queries_info);
    std::size_t queries_count = static_cast<std::size_t>(queries_info.shape[0]);
    key_t const* queries_begin = key_data<key_t>(queries_info);
...
template <typename index_at>
static py::dict join_index(                         //
    index_at const& a, index_at const& b,           //
    std::size_t max_proposals, bool exact,          //
    progress_func_t const& progress) {

    using key_t = typename index_at::vector_key_t;
    std::unordered_map<key_t, key_t, hash_gt<key_t>> a_to_b;
    ...
    py::dict result_mapping;
    for (auto const& pair : a_to_b)
        result_mapping[key_codec_gt<key_t>::to_object(pair.first)] = key_codec_gt<key_t>::to_object(pair.second);
    return result_mapping;
}

template <typename index_at> static index_at copy_index(index_at const& index, bool force_copy) {

    using copy_result_t = typename index_at::copy_result_t;
...
template <typename index_at>
static void compact_index(index_at& index, std::size_t threads, progress_func_t const& progress) {
...
template <typename index_at>
static py::class_<index_at, std::shared_ptr<index_at>> bind_dense_index(py::module_& m, char const* class_name) {

    using key_t = typename index_at::vector_key_t;
    using labeling_result_t = typename index_at::labeling_result_t;

    auto i = py::class_<index_at, std::shared_ptr<index_at>>(m, class_name);

    i.def(                                                                      //
        py::init(&make_index<index_at>),                                        //
        py::kw_only(),                                                          //
        py::arg("ndim") = 0,                                                    //
    ...
    i.def(                                            //
        "add_many", &add_many_to_index<index_at>,     //
        py::arg("keys"),                              //
        py::arg("vectors"),                           //
        py::kw_only(),                                //
        py::arg("copy") = true,                       //
    ...
    i.def(                                                  //
        "search_many", &search_many_in_index<index_at>,     //
        py::arg("queries"),                                 //
        py::arg("count") = 10,                              //
        py::arg("exact") = false,                           //
        py::arg("threads") = 0,                             //
    ...
    i.def(                                                 //
        "cluster_vectors", &cluster_vectors<index_at>,     //
        py::arg("queries"),                                //
        py::arg("min_count") = 0,                          //
        py::arg("max_count") = 0,                          //
        py::arg("threads") = 0,                            //
    ...
    i.def(                                           //
        "cluster_keys", &cluster_keys<index_at>,     //
        py::arg("queries"),                          //
        py::arg("min_count") = 0,                    //
        py::arg("max_count") = 0,                    //
        py::arg("threads") = 0,                      //
    ...
    i.def(
        "rename_one_to_one",
        [](index_at& index, py::object from, py::object to) -> bool {
            key_t from_key = key_codec_gt<key_t>::from_object(from);
            key_t to_key = key_codec_gt<key_t>::from_object(to);
            labeling_result_t result = index.rename(from_key, to_key);
            forward_error(result);
    ...
    i.def(
        "rename_many_to_many",
        [](index_at& index, py::buffer from, py::buffer to) -> std::vector<bool> {
            py::buffer_info from_info = from.request();
            py::buffer_info to_info = to.request();
            key_codec_gt<key_t>::validate_keys_buffer(from_info);
            key_codec_gt<key_t>::validate_keys_buffer(to_info);

            if (from_info.shape[0] != to_info.shape[0])
                throw std::invalid_argument("Sizes of `from` and `to` arrays don't match!");

            key_t const* from_keys = key_data<key_t>(from_info);
            key_t const* to_keys = key_data<key_t>(to_info);
            std::vector<bool> results(static_cast<std::size_t>(from_info.shape[0]), false);
            for (std::size_t i = 0; i != results.size(); ++i) {
                labeling_result_t result = index.rename(from_keys[i], to_keys[i]);
    ...
    i.def(
        "rename_many_to_one",
        [](index_at& index, py::buffer from, py::object to) -> std::vector<bool> {
            py::buffer_info from_info = from.request();
            key_codec_gt<key_t>::validate_keys_buffer(from_info);
            ...
            std::vector<bool> results(static_cast<std::size_t>(from_info.shape[0]), false);
            for (std::size_t i = 0; i != results.size(); ++i) {
                labeling_result_t result = index.rename(from_keys[i], to_key);
                results[i] = result.completed;
                forward_error(result);
            }
            return results;
    ...
    i.def(
        "remove_one",
        [](index_at& index, py::object key, bool compact, std::size_t threads) -> bool {
            key_t key_value = key_codec_gt<key_t>::from_object(key);
            labeling_result_t result = index.remove(key_value);
            forward_error(result);
            if (!compact)
                return result.completed;
    ...
    i.def(
        "remove_many",
        [](index_at& index, py::buffer keys, bool compact, std::size_t threads) -> std::size_t {
            py::buffer_info keys_info = keys.request();
            key_codec_gt<key_t>::validate_keys_buffer(keys_info);
            key_t const* begin = key_data<key_t>(keys_info);
            key_t const* end = begin + keys_info.shape[0];

            labeling_result_t result = index.remove(begin, end);
            forward_error(result);
            if (!compact)
                return result.completed;
    ...
    i.def("__len__", &index_at::size);
    i.def_property_readonly("size", &index_at::size);
    i.def_property_readonly("multi", &index_at::multi);
    i.def_property_readonly("connectivity", &index_at::connectivity);
    i.def_property_readonly("capacity", &index_at::capacity);
    i.def_property_readonly("ndim", [](index_at const& index) -> std::size_t { return index.metric().dimensions(); });
    i.def_property_readonly("dtype", [](index_at const& index) -> scalar_kind_t { return index.scalar_kind(); });
    i.def_property_readonly("serialized_length", [](index_at const& index) -> std::size_t { return index.serialized_length(); });
    i.def_property_readonly("memory_usage", &index_at::memory_usage);

    i.def_property("expansion_add", &index_at::expansion_add, &index_at::change_expansion_add);
    i.def_property("expansion_search", &index_at::expansion_search, &index_at::change_expansion_search);

    i.def(
        "change_metric",
        [](index_at& index, metric_kind_t metric_kind, metric_punned_signature_t metric_signature,
           std::uintptr_t metric_uintptr) {
    ...
    i.def_property_readonly("hardware_acceleration", [](index_at const& index) -> py::str { return index.metric().isa_name(); });

    i.def("contains_one", [](index_at const& index, py::object key) -> bool {
        return index.contains(key_codec_gt<key_t>::from_object(key));
    });
    i.def("count_one", [](index_at const& index, py::object key) -> std::size_t {
        return index.count(key_codec_gt<key_t>::from_object(key));
    });

    i.def( //
        "contains_many",
        [](index_at const& index, py::buffer keys_py) -> py::array_t<bool> {
            py::buffer_info keys_info = keys_py.request();
            key_codec_gt<key_t>::validate_keys_buffer(keys_info);

            py::array_t<bool> results_py(keys_info.shape[0]);
            auto results_py1d = results_py.template mutable_unchecked<1>();
            key_t const* keys = key_data<key_t>(keys_info);
            for (Py_ssize_t task_idx = 0; task_idx != keys_info.shape[0]; ++task_idx)
                results_py1d(task_idx) = index.contains(keys[task_idx]);
            return results_py;
        });

    i.def( //
        "count_many",
        [](index_at const& index, py::buffer keys_py) -> py::array_t<std::size_t> {
            py::buffer_info keys_info = keys_py.request();
            key_codec_gt<key_t>::validate_keys_buffer(keys_info);

            py::array_t<std::size_t> results_py(keys_info.shape[0]);
            auto results_py1d = results_py.template mutable_unchecked<1>();
            key_t const* keys = key_data<key_t>(keys_info);
            for (Py_ssize_t task_idx = 0; task_idx != keys_info.shape[0]; ++task_idx)
                results_py1d(task_idx) = index.count(keys[task_idx]);
            return results_py;
        });

    i.def( //
        "pairwise_distances",
        [](index_at const& index, py::buffer left_py, py::buffer right_py) -> py::array_t<distance_t> {
            py::buffer_info left_info = left_py.request();
            py::buffer_info right_info = right_py.request();
            key_codec_gt<key_t>::validate_keys_buffer(left_info);
            key_codec_gt<key_t>::validate_keys_buffer(right_info);
            if (left_info.shape[0] != right_info.shape[0])
                throw std::invalid_argument("Sizes of `left` and `right` arrays don't match!");

            py::array_t<distance_t> results_py(left_info.shape[0]);
            auto results_py1d = results_py.template mutable_unchecked<1>();
            key_t const* left = key_data<key_t>(left_info);
            key_t const* right = key_data<key_t>(right_info);
            for (Py_ssize_t task_idx = 0; task_idx != left_info.shape[0]; ++task_idx)
                results_py1d(task_idx) = index.distance_between(left[task_idx], right[task_idx]).min;
            return results_py;
        });

    i.def("pairwise_distance", [](index_at const& index, py::object left, py::object right) -> distance_t {
        return index.distance_between(key_codec_gt<key_t>::from_object(left), key_codec_gt<key_t>::from_object(right)).min;
    });

    i.def("get_many", &get_many<index_at>, py::arg("keys"), py::arg("dtype") = scalar_kind_t::f32_k);

    i.def(
        "get_keys_in_slice",
        [](index_at const& index, std::size_t offset, std::size_t limit) -> py::array {
            limit = std::min(index.size(), limit);
            py::array result_py = make_keys_array<key_t>(py_shape_t {static_cast<Py_ssize_t>(limit)});
            py::buffer_info result_info = result_py.request();
            index.export_keys(key_data<key_t>(result_info), offset, limit);
            return result_py;
        },
    ...
    i.def(
        "get_keys_at_offsets",
        [](index_at const& index, py::array_t<Py_ssize_t> const& offsets_py) -> py::array {
            py::array result_py = make_keys_array<key_t>(py_shape_t {offsets_py.size()});
            py::buffer_info result_info = result_py.request();
            key_t* out = key_data<key_t>(result_info);

            auto offsets_py1d = offsets_py.template unchecked<1>();
            for (Py_ssize_t task_idx = 0; task_idx != offsets_py.size(); ++task_idx)
                index.export_keys(&out[task_idx], offsets_py1d(task_idx), 1);
            return result_py;
        },
        py::arg("offsets"));
    ...
    i.def(
        "get_key_at_offset",
        [](index_at const& index, std::size_t offset) -> py::object {
            key_t result;
            index.export_keys(&result, offset, 1);
            return key_codec_gt<key_t>::to_object(result);
        },
        py::arg("offset"));
    ...
    i.def("reset", &reset_index<index_at>);
    i.def("clear", &clear_index<index_at>);
    i.def("copy", &copy_index<index_at>, py::kw_only(), py::arg("copy") = true);
    i.def("compact", &compact_index<index_at>, py::arg("threads"), py::arg("progress") = nullptr);
    i.def("join", &join_index<index_at>, py::arg("other"), py::arg("max_proposals") = 0, py::arg("exact") = false,
          py::arg("progress") = nullptr);

    i.def_property_readonly("max_level", &max_level<index_at>);
    i.def_property_readonly("stats", &compute_stats<index_at>);
    i.def_property_readonly("levels_stats", &compute_levels_stats<index_at>);
    i.def("level_stats", &compute_level_stats<index_at>, py::arg("level"));
...
PYBIND11_MODULE(compiled, m) {
    ...
    bind_dense_index<dense_index_py_t>(m, "Index");
    bind_dense_index<dense_index_big_py_t>(m, "IndexBig");
    ...
    is.def("merge_paths", &dense_indexes_py_t::merge_paths, py::arg("paths"), py::arg("view") = true,
           py::arg("threads") = 0);
    is.def(                                                            //
        "search_many", &search_many_in_indexes<dense_indexes_py_t>,    //
        py::arg("query"),                                              //
        py::arg("count") = 10,                                         //
        py::arg("exact") = false,                                      //
        py::arg("threads") = 0,                                        //
    ...
    auto isb = py::class_<dense_indexes_big_py_t>(m, "IndexesBig");
    isb.def(py::init());
    isb.def("__len__", &dense_indexes_big_py_t::size);
    isb.def("merge", &dense_indexes_big_py_t::merge);
    isb.def("merge_paths", &dense_indexes_big_py_t::merge_paths, py::arg("paths"), py::arg("view") = true,
            py::arg("threads") = 0);
    isb.def(                                                                //
        "search_many", &search_many_in_indexes<dense_indexes_big_py_t>,     //
        py::arg("query"),                                                   //
        py::arg("count") = 10,                                              //
        py::arg("exact") = false,                                           //
        py::arg("threads") = 0,                                             //
